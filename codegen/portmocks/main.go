...
func (store *GenStore) Generate(out *os.File) {
...
type YamlConfig struct {
	...
	Package            string                        `yaml:"package"`
	MockPackage        string                        `yaml:"mock"`
	EmbeddedMockStruct string                        `yaml:"embedded_mock"`
	Generate           map[string]YamlGenerateConfig `yaml:"generate"`
}

func (c *YamlConfig) ReadConfig(file string) {
...
func main() {
	...
	for fileout, fileoutConfig := range conf.Generate {
		config := Config{
			Package:            conf.Package,
			MockPackage:        conf.MockPackage,
			LocalPackage:       fileoutConfig.Package,
			EmbeddedMockStruct: conf.EmbeddedMockStruct,
			Debug:              conf.Debug,
		}

		process(config, fileoutConfig.Source, fileout)
	}
}
